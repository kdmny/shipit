...
Shipit.prototype.remote = function (command, options, cb, parallel) {
  if(parallel){
    return this.pool.run(command, options, cb);
  }
  console.log("running remote serially...");
  ...
  return this.pool.connections.reduce(function (promise, connection) {
    return promise.then(function(result) {
      return connection.run(command, options);
    });
  }, new Promise.resolve()).nodeify(cb);
};
